...
1. **Locate.** `screenshot()` to see the desktop and find your target
   visually.
2. **Resolve coordinates.** `screenshot(region=Region(x, y, w, h),
   detect=True)` on the target area. The annotated image labels every
   detected element with its center point — those numbers are the literal
   click point. Read them; never estimate.
...
coordinates. Re-zoom the target area for a fresh detection and use the
new coordinates.
