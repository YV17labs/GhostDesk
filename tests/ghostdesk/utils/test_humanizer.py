...
"""Tests for ghostdesk.utils.humanizer — human-like mouse and typing simulation."""
...
from ghostdesk.utils.humanizer import (
    _bezier_point,
    _ease_in_out,
...
# ---------------------------------------------------------------------------
# Async function tests (mock ghostdesk.utils.cmd.run)
# ---------------------------------------------------------------------------
...
class TestHumanMove:
    ...
    async def test_calls_xdotool_mousemove(self):
        ...
        with patch("ghostdesk.utils.humanizer.run", new_callable=AsyncMock) as mock_run, \
