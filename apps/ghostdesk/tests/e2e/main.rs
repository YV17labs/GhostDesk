...
//! Sway up, which is what the supervisord stack brings; on macOS and Windows it
//! means the session you are already logged into, because `host::input()` and
//! its four siblings bind Quartz and the Accessibility API — `SendInput`, GDI
//! and `EnumWindows` on Windows — directly, with no stack in between.
//!
