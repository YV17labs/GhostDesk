...
- **Every environment variable is renamed, and there is now only one name for each.** The deployment sets `NESTRS_ENV_PREFIX=GHOSTDESK` — both images bake it, the `Justfile` exports it — so the framework reads `GHOSTDESK_HTTP__PORT` where a stock NestRS app would read `NESTRS_HTTP__PORT`. The entrypoint's `GHOSTDESK_* → NESTRS_*` translation layer is gone with it: what you set is what the server reads. Settings are `#[config]` structs validated at boot, one per feature module rather than one catch-all — `screen`, `session` and `auth` alongside the framework's own `http` and `mcp`. Read a name as `GHOSTDESK_<NAMESPACE>__<KEY>`; a single underscore (`GHOSTDESK_VNC_PASSWORD`) marks a container knob the entrypoint consumes itself.
  ...
  | `GHOSTDESK_IDLE_TIMEOUT` | `GHOSTDESK_SESSION__IDLE_TIMEOUT_SECS` |
...
- **A malformed setting now fails the boot** naming the variable, instead of quietly falling back to a default. `GHOSTDESK_SESSION__IDLE_TIMEOUT_SECS=18OO` used to be indistinguishable from deliberately choosing 1800.
