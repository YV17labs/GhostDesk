...
### Changed
- **The server is a single binary.** `apps/ghostdesk` wires modules and owns no logic; `crates/features` holds one folder per domain (screen, input, apps, clipboard, auth, idle, telemetry) — a `module.rs` that registers it, a `service.rs`, or a `services/` directory when there is more than one — plus the single `#[mcp(path = "/mcp")]` host that publishes all of them, and nothing but the `*Module` types is public; `crates/platform` is the OS substrate (Wayland, Sway IPC, `grim`, `.desktop`, coordinate space) and carries no framework types. The whole dependency graph is verified at boot — a missing provider fails the boot naming it, rather than surfacing on the first tool call.
- **The OS boundary is five traits.** `InputBackend`, `ScreenBackend`, `WindowManager`, `Clipboard` and `AppCatalog` are the only doors between the feature layer and the desktop. Each OS is a directory under `crates/platform/src/` whose modules mirror those five contracts by name, and `platform::host` is the single point where one is chosen. Window identity is opaque per backend — Sway's `con_id` is an implementation detail, not the contract — and no compositor JSON ever crosses into the feature crate. Adding an OS is a new sibling directory plus one arm per selector; nothing in `features` or `mcp` names an operating system.
...
- **Every environment variable is renamed, and there is now only one name for each.** The deployment sets `NESTRS_ENV_PREFIX=GHOSTDESK` — both images bake it, the `Justfile` exports it — so the framework reads `GHOSTDESK_HTTP__PORT` where a stock NestRS app would read `NESTRS_HTTP__PORT`. The entrypoint's `GHOSTDESK_* → NESTRS_*` translation layer is gone with it: what you set is what the server reads. Settings are `#[config]` structs validated at boot, one per feature module rather than one catch-all — `screen`, `idle`, `telemetry` and `auth` alongside the framework's own `http` and `mcp`. Read a name as `GHOSTDESK_<NAMESPACE>__<KEY>`; a single underscore (`GHOSTDESK_VNC_PASSWORD`) marks a container knob the entrypoint consumes itself.
  ...
  `GHOSTDESK_TLS_CERT` / `_KEY`, `GHOSTDESK_VNC_PASSWORD`, `GHOSTDESK_USER` and `GHOSTDESK_DIR` keep their names — the entrypoint reads them, not the server.
...
- **Launched apps are scrubbed of the entire `GHOSTDESK_*` namespace**, not just the two names the Python port listed. It carries the bearer token and inline TLS key material, and a browser the agent spawns has no business with any of it. One prefix rather than a list of names means a setting added later cannot be forgotten here.
